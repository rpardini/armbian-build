...
function cli_obtain_complete_artifact() {
	...
	# @TODO: NOT/NOT-ONLY here.
	# This will take from local cache, which is definitely present at this point, and re-version it according to
	# $REVISION. Each artifact might need to do this differently, so each artifact has its own implementation,
	# although most will be happy wit the standard "standard_artifact_reversion_for_deployment()".
