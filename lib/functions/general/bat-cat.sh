...
function run_tool_batcat() {
	...
	display_alert "MACHINE" "${MACHINE}" "warn"

	case "$MACHINE" in
		*x86_64-pc-linux-gnu*) BATCAT_ARCH_OS="x86_64-unknown-linux-gnu" ;;
		*)
			exit_with_error "unknown os/arch: $MACHINE"
			;;
	esac
	...
	if [[ ! -f "${DIR_BATCAT}/cache/syntaxes.bin" ]]; then
		...
		BAT_CONFIG_DIR="${DIR_BATCAT}/config" BAT_CACHE_PATH="${DIR_BATCAT}/cache" "${BATCAT_BIN}" cache --build
	fi
	...
	display_alert "Calling batcat" "$*" "debug"
	BAT_CONFIG_DIR="${DIR_BATCAT}/config" BAT_CACHE_PATH="${DIR_BATCAT}/cache" "${BATCAT_BIN}" --paging=never --force-colorization --style full --wrap auto --terminal-width 120 "$@"
}
