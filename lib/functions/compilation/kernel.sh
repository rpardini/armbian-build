...
function kernel_config() {
	...
	# commit new .config, as if it was a patch applied.
	# .config is included in .gitignore, so we have to force git to add it here too.
	# report it first to increase fasthash as well.
	report_fasthash file "${kernel_work_dir}/.config" "Kernel configuration ${LINUXCONFIG}"
	regular_git add -f .config
	mark_fasthash_done "${kernel_config_mtime}"
}
