...
function kernel_download_bundle_with_axel() {
	...
	run_host_command_logged axel --verbose --alternate "--output=${linux_kernel_clone_bundle_file_tmp}" "${linux_clone_bundle_url}"
}
...
function kernel_prepare_bare_repo_from_bundle() {
	...
	if [[ ! -d "${kernel_git_bare_tree}" || ! -f "${kernel_git_bare_tree_done_marker}" ]]; then
		...
		git clone --progress --verbose --tags --no-checkout "${linux_kernel_clone_bundle_file}" "${kernel_git_bare_tree}"
...
function kernel_prepare_git() {
	...
	# @TODO: this is not the place to do cleanups. Move somewhere else.
	...
	# conundra with the mountpoint here
	#if [[ -d "${SRC}/cache/sources/linux-kernel" ]]; then
	#	display_alert "Cleaning up old linux-kernel sources directory" "might take a while 2" "debug"
	#	run_host_command_logged rm -rf "${SRC}/cache/sources/linux-kernel"/*
	#fi
