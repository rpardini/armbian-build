function aggregate_all_packages() {
	#### Aggregation stage 1:
	#### ---- CUT: Run Python for initial aggregation here.
	#### It should spit out: ALL ARRAYS...
	#### DEBOOTSTRAP_COMPONENTS
	#### DEBOOTSTRAP_LIST
	#### PACKAGE_LIST
	#### PACKAGE_LIST_ADDITIONAL
	#### PACKAGE_LIST_EXTERNAL
	#### PACKAGE_LIST_DESKTOP
	#### Namely, no removals are done at this stage.
	#### Hooks are allowed to modify the arrays soon.
	...
	declare -a aggregation_params_quoted=(
		...
		# - AGGREGATED_PACKAGES_ROOTFS (which includes AGGREGATED_PACKAGES_DEBOOTSTRAP)
	...
	run_host_command_logged env -i "${aggregation_params_quoted[@]@Q}" python3 "${SRC}/lib/tools/release_package_aggregation.py"
	display_alert "wtf" "wtf" "info"
