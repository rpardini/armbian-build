...
from tools.common.matrix_utils import MatrixInput, MatrixKernel, MatrixUboot
...
# Instantiate MatrixKernel objects and add them to the kernels list
uboots = [MatrixUboot(ub_id, entries[0], entries) for ub_id, entries in grouped_by_uboot_id.items() if ub_id is not None]
# Sort u-boots by the number of all_items
uboots.sort(key=lambda k: len(k.all_items), reverse=True)

log.info(f"Parsed {len(uboots)} u-boots")
for uboot in uboots:
	log.info(f"{uboot}")
