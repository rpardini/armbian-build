...
function extension_prepare_config__be_more_awesome() {
...
function post_build_image__920_create_ovf() {
	...
	qemu-img create -f qcow2 -F qcow2 -b "${original_qcow2_image}" "${temp_qcow2_image}" &> /dev/null # create a new, temporary, qcow2 with the original as backing image
	qemu-img resize "${temp_qcow2_image}" +47G &> /dev/null                                           # resize the temporary
	qemu-img convert -f qcow2 -O vmdk "${temp_qcow2_image}" "${full_file_vmdk}" &> /dev/null          # convert the big temp to vmdk
	...
	# Now wrap the .vmx in a zip, with minimal compression. (release will .xz or .zst it later)
	display_alert "Zipping/storing vmx" "${EXTENSION}" "info"
	cd "${DESTIMG}" || false
	zip -0 "${final_vmx_zip_file}" "${base_vmware_dirname}"/*
	cd - || false

	display_alert "Done, cleaning up" "${EXTENSION}" "info"
	rm -rf "${full_vmware_dirname}" "${temp_qcow2_image}"
}
