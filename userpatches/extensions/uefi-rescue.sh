...
user_config__700_rescue_uefi_arm64_config() {
	export CLOUD_INIT_EXTRA_VERSION="rescue"
	...
	export DONT_EXPORT_REGULAR_IMG="yes"                  # No use for a non-e2img at the output, it won't ever be used.
	export EXTRA_ROOTFS_MIB_SIZE=256                      # arm64 image requires a bit more space for Grub?
	export SERIALCON="ttyAMA0"                            # These are mostly used in cloud environments, where a console exists.
	[[ "${BOARD}" == *x86* ]] && export SERIALCON="ttyS0" # For Oracle VMs on AMD micro stuff
