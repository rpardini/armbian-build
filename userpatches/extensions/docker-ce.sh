...
function pre_customize_image__add_docker_to_image() {
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]]; then
		...
		run_host_command_logged echo "deb [arch=${ARCH} signed-by=/usr/share/keyrings/docker.gpg] https://download.docker.com/linux/debian bookworm stable" "|" tee "${SDCARD}"/etc/apt/sources.list.d/docker.list
