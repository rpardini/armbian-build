...
function extension_prepare_config__amazingfated_rk3588() {
	...
	if [[ "${LINUXFAMILY}" != "rockchip-rk3588" ]]; then
		exit_with_error "${EXTENSION} only works on LINUXFAMILY=rockchip-rk3588, currently on '${LINUXFAMILY}'"
	fi
...
function post_install_kernel_debs__amazingfated_rk358() {
	...
	do_with_retries 3 chroot_sdcard_apt_get_install rockchip-multimedia-config mali-g610-firmware chromium-browser
