function extension_prepare_config__java_build_machine() {
	...
	if [[ ${EXTRA_ROOTFS_MIB_SIZE} -le 1024 ]]; then
		export EXTRA_ROOTFS_MIB_SIZE=1024
		display_alert "GraalVM: Setting new EXTRA_ROOTFS_MIB_SIZE: ${EXTRA_ROOTFS_MIB_SIZE}" "${EXTENSION}" "info"
	fi

	export GRAALVM_ARCH="${ARCH}"
	[[ "${ARCH}" == "arm64" ]] && export GRAALVM_ARCH="aarch64"

	export GRAALVM_VERSION="22.2.0"
	export GRAALVM_FILENAME="graalvm-ce-java17-linux-${GRAALVM_ARCH}-${GRAALVM_VERSION}.tar.gz"

	export GRAALVM_URL="https://github.com/graalvm/graalvm-ce-builds/releases/download/vm-${GRAALVM_VERSION}/${GRAALVM_FILENAME}"
	export GRAALVM_CACHE_DIR="${SRC}/cache/graalvm"
	export GRAALVM_CACHE_FILE="${GRAALVM_CACHE_DIR}/${GRAALVM_FILENAME}"
