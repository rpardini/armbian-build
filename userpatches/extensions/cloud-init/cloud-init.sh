...
extension_prepare_config__990_late_finish_cloud_init_config() {
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]] && [[ "${CLOUD_INIT_USE_NETPLAN}" == "yes" ]]; then
		display_alert "${DISTRIBUTION} requires ifupdown for cloud-init" "${DISTRIBUTION}" "info" # @TODO: does it?
	...
	EXTRA_WANTED_PACKAGES="lvm2 thin-provisioning-tools systemd-timesyncd wpasupplicant" # networkd-dispatcher
