...
extension_prepare_config__990_late_finish_cloud_init_config() {
	...
	remove_packages_everywhere network-manager-openvpn network-manager hostapd ${CLOUD_INIT_NETWORK_PACKAGE_REMOVE}
}
...
pre_umount_final_image__300_prepare_cloud_init_startup() {
	...
	display_alert "Configuring cloud-init at" "${CLOUD_INIT_CONFIG_LOCATION}" "wrn"
...
pre_customize_image__restore_preserved_systemd_and_netplan_stuff() {
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]]; then
		...
		wget --quiet --output-document="${SDCARD}/etc/cloud/templates/sources.list.debian.tmpl" "https://raw.githubusercontent.com/canonical/cloud-init/main/templates/sources.list.debian.tmpl" || display_error "Failed to update c-i apt template for" "${RELEASE}" "err"
	fi
}
