...
function extension_prepare_config__950_prepare_cloud_init() { # do it very late so others can set their stuff first
	...
	if [[ "${DISTRIBUTION}" == "Debian" ]]; then
		declare -g CLOUD_INIT_USE_NETPLAN="${CLOUD_INIT_USE_NETPLAN:-no}" # use NetworkManager.
	else
		declare -g CLOUD_INIT_USE_NETPLAN="${CLOUD_INIT_USE_NETPLAN:-yes}" # use netplan.io and the systemd/networkd renderer.
	fi
...
function extension_prepare_config__990_late_finish_cloud_init_config() {
	...
	if [[ "${CLOUD_INIT_USE_NETPLAN}" == "yes" ]]; then
		...
		display_alert "Using network-manager with ifupdown" "${EXTENSION} ${DISTRIBUTION}:${RELEASE}" "info"
