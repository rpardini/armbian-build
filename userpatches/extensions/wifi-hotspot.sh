...
function pre_customize_image__068_setup_improv_wifi() {
	...
	cat <<- SYSTEMD_IMPROV_WIFI > "${SDCARD}"/usr/lib/systemd/system/improv-wifi.service
		...
		After=early-autoconfig.service bluetooth.service
		Requires=early-autoconfig.service bluetooth.service
...
function create_avahi_service_with_uuid_and_name() {
	# Important: Avahi reloads config if the service file is changed, even if it is identical to the previous one.
	# If this happens exactly during network change, it can cause a bug.
	# @TODO: Maybe check for changes before writing.

	# Load Armbian version et al from release (for ${VERSION and ${BOARD}}
	. /etc/armbian-release
	...
	cat <<- EOD > "/etc/avahi/services/${VENDOR_AVAHI_NAME}.service"
		...
		<service-group>
			...
			<service>
				...
				<txt-record value-format="text">firmwareversion=${VERSION}</txt-record>
				<txt-record value-format="text">board=${BOARD}</txt-record>
			</service>
		</service-group>
	EOD
