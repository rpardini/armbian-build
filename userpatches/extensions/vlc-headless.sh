...
function pre_customize_image__005_vlc_service() {
	...
	cat <<- EOD > "${SDCARD}/usr/local/sbin/vlc-alsa-launcher.sh"
		...
		# Now, wait for the UUID to be set...
		if [[ ! -f "${VENDOR_INFO_DIR}/uuid.set" ]]; then
			echo "UUID is not set yet, wait..."
			sleep 3
			exit 121
		fi
		...
		if [[ "x\${VLC_ALSA_DEVICE}x" != "xx" ]]; then
			...
			# speaker test, disabled.
			#timeout 60 speaker-test -D"\${VLC_ALSA_DEVICE}" -c 8 -t wav --nloops 1 || true
			#sleep 2
			exec cvlc --alsa-audio-channels 4967 --alsa-audio-device "\${VLC_ALSA_DEVICE}" --http-password "vlcremote" --no-metadata-network-access --no-auto-preparse --intf http --no-dbus --no-daemon --verbose 1 ${VLC_MEDIA_START_PLAYLIST}
		else
